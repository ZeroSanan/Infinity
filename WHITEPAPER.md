...
### Market Signals Tab
- Live regime read for BTC, ETH, XRP, SOL, ZEC, SPCX, SUI, XAU (gold), and NVDA on the 4-hour timeframe, refreshed every 15 minutes
...
In addition to the DCA trading engines, Infinity runs a real-time **Market Signals** system (`/api/market/signals`) that continuously evaluates **BTC, ETH, XRP, SOL, ZEC, SPCX, SUI** (crypto, via Binance) plus **XAU (gold) and NVDA** (via Twelve Data) on the 4-hour timeframe. For each asset it produces:
...
|-------|--------|--------|-------|
| BTC, ETH, XRP, SOL, ZEC, SPCX, SUI | Binance public klines (`GET /api/v3/klines`, `interval=4h`, `limit=200`) | `BTCUSDT`, etc. | No API key required |
...
| 4h klines (200 candles ≈ 33 days) | `GET /api/v3/klines` (BTC/ETH/XRP/SOL/ZEC/SPCX/SUI) | 4h | Market Signals (Section 11.2) and ML models (Section 11.6) for all seven; Fibonacci levels (Section 11.4) for BTC/ETH/XRP/SOL only | Recomputed every 15 min (`_SIGNALS_TTL`) by the background signal poller |
